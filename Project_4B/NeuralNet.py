...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):                                       
        ...
        """YOUR CODE"""
        bets = [1.0] + inActs
        returnVal = self.sigmoid(self.getWeightedSum(bets))
        return returnVal
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        totalModification = 0
        inActs = [1.0] + inActs
        for x in enumerate(self.weights):
            i = x[0]
            weight = x[1] 
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            nextActs = []
            for i in layer:
                value = i.sigmoidActivation(currentActs)
    ...
    def backPropLearning(self, examples, alpha):
        ...
        for example in examples:
            ...
            temp = example[0]
            temp1 = example[1]
            allLayerOutput = self.feedForward(temp)
            ...
            for outputNum in xrange(len(example[1])):
                ...
                error = temp1[outputNum] - lastLayerOutput[outputNum] 
                delta = gPrime * error 
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    tent = 0
                    for i in nextLayer:
                        summy += i.weights[neuronNum+1] * deltas[0][tent]
                        tent = tent + 1
                    delta = summy * gPrime
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    for example in examplesTest:
        ...
        fullOutput = nnet.feedForward(first)
        outerLayerOutput = [round(output) for output in fullOutput[-1]]

        if outerLayerOutput != second:
            testError += 1
        else:
            testCorrect += 1
