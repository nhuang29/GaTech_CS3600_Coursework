        ...
        if noisyDistance != None:
            for pos in legalPos:
                ...
                if not emissionModel[trueDistance] < 0:
                    result[pos] = self.beliefs[pos] * emissionModel[trueDistance]
        else:
            result[x] = 1.0
            for pos in legalPos:
                result[pos] = 0.0
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for pos in legalPos:
            ...
            item = newPosDist.items()
            for i in item:
                newPos = i[0]
                prob = i[1]
                add = self.beliefs[pos] * prob
                result[newPos] = result[newPos] + add
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        particulars = self.numParticles
        counter = 0
        for pointer in range(1, particulars + 1):

            pos = self.legalPositions[counter]
            self.particles.append(pos)

            if not (counter + 1) >= numLegalPos:
                if not (pointer % (particulars / numLegalPos) != 0):
                    counter = counter + 1
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        result = util.Counter()
        particulars = self.particles
        numPart = self.numParticles
        if not noisyDistance is None:
            for x in particulars:
                truth = util.manhattanDistance(pacmanPosition, x)
                if not result[x] is None:
                    result[x] = result[x] + emissionModel[truth]
                else:
                    result[x] = emissionModel[x]
            ...
            if not totals != 0:
                ...
                for i in range(0, numPart):
                    myList.append(util.sample(result))
    ...
    def elapseTime(self, gameState):
        ...
        i = 0
        particulars = self.particles
        numLen = len(particulars)
        while i < numLen:
            pos = self.getPositionDistribution(self.setGhostPosition(gameState, particulars[i])) 
            self.particles[i] = util.sample(pos)
            i = i + 1
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        legalPos = self.legalPositions
        lenLegal = len(legalPos)
        while not counter > self.numParticles:
            for z in legalPos:
                self.particles = self.particles + [tuple(z for x in range(numGhost))]
                i = i + 1
                if not i != lenLegal:
                    i = 0
                counter = counter + 1
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        "*** YOUR CODE HERE ***"
        weights = [util.Counter() for d in range(0, self.numGhosts)]
        result = util.Counter()
        ...
        z = 0
        while z < ghosts:
            while not weights[z].totalCount() != 0:
                for part in self.particles:
                    truth = util.manhattanDistance(part[z], pacmanPosition)
                    weights[z][part[z]] = weights[z][part[z]] + emissionModels[z][truth]

                if noisyDistances[z] == None:
                    break

                if not weights[z].totalCount() != 0:
                    self.initializeParticles()
            z = z + 1
        ...
        while x < numParts:
            ...
            while i < ghosts:
                if not noisyDistances[i] is None:
                    utilSamp = util.sample(weights[i])
                    plate.append(utilSamp)
                else:
                    jailer = self.getJailPosition(i)
                    plate.append(jailer)
                i = i + 1
            x = x + 1
            newTup = tuple(plate)
            update.append(newTup)
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            x = 0
            while x < ghost:
                temp = setGhostPositions(gameState, newParticle)
                newPosDist = getPositionDistributionForGhost(temp, x, self.ghostAgents[x])
