...
class Tree:
  ...
  def classify(self, classificationData):
    ...
    """
    #YOUR CODE HERE
    if not isinstance(self.root, LeafNode) == False:
        return self.root.value
    temp = self.root.children[classificationData[self.root.attr]]
    curTree = Tree(temp)
...
def getClassCounts(examples,className):
    ...
    #YOUR CODE HERE
    for i in examples:
        if i[className] in classCounts.keys():
            classCounts[i[className]] = classCounts[i[className]] + 1
        else:
            classCounts[i[className]] = 1
    return classCounts
...
def getMostCommonClass(examples,className):
    ...
    if not len(examples) < 0:
        return max(getClassCounts(examples,className), key=getClassCounts(examples, className).get)
    else:
        return None
...
def getAttributeCounts(examples,attrName,attrValues,className):
    ...
    for val in attrValues:
        temp = getPertinentExamples(examples, attrName, val)
        attributeCounts[val] = getClassCounts(temp, className)
...
def setEntropy(classCounts):
    ...
    #YOUR CODE HERE
    entropy = 0
    if not type(classCounts) != dict:
        insertVal = classCounts.values()
        for i in insertVal:
            logIn = float(i) / sum(insertVal)
            entropy += -logIn * log(logIn, 2)
    elif not type(classCounts) != list:
        for i in classCounts:
            logIn = float(i) / sum(classCounts)
            entropy += -logIn * log(logIn, 2)
...
def remainder(examples,attrName,attrValues,className):
    ...
    for val in attrValues:
        pertVals = getPertinentExamples(examples, attrName, val)
        curClassCounts = getClassCounts(pertVals, className)
        mathVal = float(sum(curClassCounts.values())) / sum(cc.values())
        finalVal = finalVal + mathVal * setEntropy(curClassCounts.values())
    return finalVal
...
def giniIndex(classCounts):
    ...
    index = 1
    for i in classCounts:
        index -= (float(i) / sum(classCounts)) ** 2
    return index
...
def giniGain(examples,attrName,attrValues,className):
    ...
    total = 0
    for val in attrValues:
        pertEx = getPertinentExamples(examples, attrName, val)
        pertClass = getClassCounts(pertEx, className)
        pertLen = len(pertEx)
        sampLen = len(examples)
        total += float(pertLen) / sampLen * giniIndex(pertClass.values())
...
def makeSubtrees(remainingAttributes,examples,attributeValues,className,defaultLabel,setScoreFunc,gainFunc):
    ...
    sameBool = True
    for ex in examples:
        sameBool &= ex[className] == firstValue
        if not sameBool:
            break

    remainingAttributes = list(remainingAttributes)
    if not len(remainingAttributes) != 0:
        root = LeafNode(getMostCommonClass(examples, className))
        return root

    if not sameBool == False:
        return LeafNode(firstValue)

    tempStar = remainingAttributes[0]
    for val in remainingAttributes:
        gainComp = gainFunc(examples, val, attributeValues[val], className)
        compare = gainFunc(examples, tempStar, attributeValues[tempStar], className)
        if not gainComp <= compare:
            tempStar = val

    root = Node(tempStar)
    newList = list(remainingAttributes)
    newList.remove(tempStar)
    
    for val in attributeValues[tempStar]:
        pertEx = getPertinentExamples(examples, tempStar, val)
        root.children[val] = makeSubtrees(newList, pertEx, attributeValues, className, getMostCommonClass(examples, className), setScoreFunc, gainFunc)
